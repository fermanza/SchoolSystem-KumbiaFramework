<?php
	class AdministradorController extends ApplicationController {
       ...
       function cambiar_contrasena($vacia){
			$this->checarusuario();
        ...
        function cambiando_contrasena(){
            ...
            if( !isset($contrasena) || $contrasena == "" )
                $this->redirect('administrador/cambiar_contrasena/1');
            // Eliminar las variables que van a pertenecer a este método.
            unset( $this -> usuario );
            unset( $this -> exito );
            ...
            foreach( $usuarios -> find_all_by_sql( "
				...
				if( $contrasena == $usuario -> passwd_old || $contrasena == $usuario -> clave ){
					$this->redirect('administrador/cambiar_contrasena/2'); return;
				}
				if( strlen($contrasena) < 6 || strlen($contrasena) > 15	){
					$this->redirect('administrador/cambiar_contrasena/3'); return;
				}
            ...
            foreach( $usuarios -> find_all_by_sql( "
					...
					passwd_old = AES_ENCRYPT('".$contrasenaAnterior."','".$semilla -> getSemilla()."'),
