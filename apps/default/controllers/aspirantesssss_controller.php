<?php
	...
	class AspirantesController extends ApplicationController {
		...
		function reporte($nivel = "I" , $carrera=400, $x=0){
		// function reporte($nivel = "" , $carrera=400, $x=0){
			$configuracion = new Configuracion();
			$configuracion = $configuracion -> find_first("periodo=12010");
		...
		function registro($folio){
			...
			$maspirantes = new Aspirantes();
			$n = $maspirantes -> count("folio=".$folio." AND periodo=".$configuracion -> periodo);
			
			if($n>0){
				$this -> redirect("aspirantes/modificacion/".$folio);
				return;
			}
		...
		function ficha($folio){
			...
			$ceneval = $mexanis -> find_first("id=114");
			...
			$reporte -> Image('http://ase.ceti.mx/tecnologo/public/img/logoceti.jpg', 5, 8);
			
			$reporte -> Image('http://ase.ceti.mx/tecnologo/public/img/fotografia.jpg', 180, 15);
			...
			$reporte -> Cell(20,4,"TC".$ficha,1,0,'C',1);
			...
			if($aspirante -> exani_id != 0){	
				...
				$reporte -> Cell(55,6,substr($exani -> lugar,0,30),1,0,'C',1);
			...
			$reporte -> SetX(20);
			$reporte -> SetFillColor(0xDD,0xDD,0xDD);
			$reporte -> Cell(60,30,"",0,0,'C',0);
			$reporte -> Cell(60,30,"",1,0,'C',1);
			$reporte -> Cell(60,30,"",1,0,'C',1);
			
			$reporte -> SetY(100);			
			$reporte -> Text(141, 77, "DOCUMENTACION RECIBIDA: ".$AspDoc -> fotos);
			$reporte -> SetY(100);			
			...
			if($AspDoc -> find_first("aspirante_id = ".$aspirante -> id)){
				...
				$reporte -> Text(141, 83, "Fotografias: ".$AspDoc -> fotos);
				$reporte -> Text(141, 86, "Constancia de Estudios: ".$AspDoc -> constanciae);
				$reporte -> Text(141, 89, "Certificado Original y Copia: ".$AspDoc -> certificadoo);
				//$reporte -> Text(141, 92, "Certificado Copia: ".$AspDoc -> certificadoc);
				$reporte -> Text(141, 92, "Acta de Nacimiento Copia: ".$AspDoc -> acta);
				$reporte -> Text(141, 95, "Const. de Buena Conducta: ".$AspDoc -> constanciab);
				$reporte -> Text(141, 98, "Certificado Medico: ".$AspDoc -> certificadom);
			}
			$reporte -> SetY(130);
			$reporte -> SetFillColor(0xFF,0xFF,0xFF);
			...
			$reporte -> Output("/var/www/htdocs/calculo/tecnologo/public/files/pdfs/aspirantes/".$ficha.".pdf");
			//$reporte -> Output("C:/Archivos de programa/VertrigoServ/www/tecnologo/public/files/pdfs/aspirantes/".$ficha.".pdf");
...
//			exit(1);
			...
			*/
			...
			$reporte -> Output("/datos/calculo/ingenieria/public/files/pdfs/aspirantes/".$ficha.".pdf");
