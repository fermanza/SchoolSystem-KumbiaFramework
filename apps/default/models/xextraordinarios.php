	...
	class Xextraordinarios extends ActiveRecord {
		...
		public function get_extrasTitulos_alumno($registro, $periodo, $plantel,$tipo)
		{
		  ...
		  if($plantel == "C")
		  {
			$extras = $xextraordinarios->find_all_by_sql("SELECT e.*,c.*,m.nombre,c.materia, a.situacion, IFNULL(ct.actas,'False') AS Agregado 
														  ...
														  LEFT JOIN xalumnocursos a ON a.curso_id = e.curso_id and a.registro = '".$registro."' AND a.periodo = '".$periodo."'
		...
		*/
		public function get_cursos_NIV_ACR($registro, $periodo, $tipoCurso)
		{
		  ...
		  $extras = $xextraordinarios->find_all_by_sql("SELECT m.nombre, ic.clavecurso,ia.tipo_ex, ia.id, m.clave, IFNULL(ct.actas,'False') AS Agregado 
														...
														WHERE ia.tipo_ex = '".$tipoCurso."' AND ic.periodo = '".$periodo."' AND ia.registro ='".$registro."' GROUP BY ic.id");
														
		  return $extras;  
		}
		
		/*
		 * Esta funcion sirve para obtener los examenes globales segun lo solicite el alumno
	     * @param int $registro Registro del alumno que solicito el pago del examen global
		 * @param int $periodo Periodo intersemestral inicia con 2 o 4 
		*/
		public function get_Examen_Global($registro, $periodo, $tipoCurso)
		{
		  
		  $xextraordinarios	= new Xextraordinarios();
		  									  
		  $extras = $xextraordinarios->find_all_by_sql("SELECT m.nombre, g.clavemat,g.id,g.tipo_ex, m.clave, IFNULL(ct.actas,'False') AS Agregado 
														FROM globales g
														JOIN materia m ON m.clave = g.clavemat
														LEFT JOIN caja_tramites ct ON ct.registro = g.registro AND ct.actas = g.id
														WHERE g.periodo = '".$periodo."' AND g.registro ='".$registro."' GROUP BY g.id");
		 ...
		 * @param int $periodo Periodo intersemestral inicia con 2 o 4 
		*/
		public function get_Exam_Global_PDF($registro, $periodo, $acta)
		{
		  ...
		  $extras = $xextraordinarios->find_all_by_sql("SELECT m.nombre, g.clavemat,g.id,g.tipo_ex, m.clave, IFNULL(ct.actas,'False') AS Agregado 
														...
														JOIN materia m ON m.clave = g.clavemat
														LEFT JOIN caja_tramites ct ON ct.registro = g.registro AND ct.actas = g.id
														WHERE g.id = '".$acta."' AND g.periodo = '".$periodo."' AND g.registro =".$registro);
		...
		*/
		...
		/*
		 * Esta funcion sirve para obtener el nombre de la materia de los intersemestrales
	     * @param int $registro Registro del alumno que solicito el pago del extraordinario
		 * @param int $periodo Periodo intersemestral inicia con 2 o 4 
		 * @param int $tipoCurso Tipo de curso que solicita ver el alumno
		*/
		public function get_cursos_NIV_ACR_INTER($registro, $periodo, $cursoID)
		{
		  $xextraordinarios	= new Xextraordinarios();
		  
		  $extras = $xextraordinarios->find_all_by_sql("SELECT ia.registro, m.nombre, ic.periodo, ic.id 
														FROM intersemestral_cursos ic 
														JOIN intersemestral_alumnos ia on ic.clavecurso=ia.clavecurso
														LEFT JOIN materia m on ic.clavemat=m.clave
														WHERE ia.registro = '".$registro."' AND  ic.id = '".$cursoID."'
														GROUP BY registro, nombre, id");
		...
		*/
		...
		/*
		 * Esta funcion sirve para obtener los extras y titulos de cualquier plantel
	     * @param int $registro Registro del alumno que solicito el pago del extraordinario
		 * @param int $periodo Periodo que esta activo
		*/
		public function get_extras_titulos_cualquier_plantel($registro, $periodo)
		{
			$extras	= array();
			foreach( $this->find_all_by_sql("SELECT e.*
											FROM xextraordinarios e
											where e.registro = '".$registro."' AND e.periodo = '".$periodo."'") as $extra){
	    ...
	    /*
		 * Esta funcion sirve para obtener los derechos de pasante que tiene un alumno
	     * @param int $registro Registro del alumno que solicito el pago del extraordinario
		 * @param int $periodo Periodo que esta activo
		*/
		public function muestra_derechoPasante($registro, $periodo)
		{
		  $xextraordinarios	= new Xextraordinarios();

		  
		  $result = $xextraordinarios->find_all_by_sql("SELECT d.idDerecho AS id, d.claveMateria AS clave, m.nombre
														FROM derechopasante d
														JOIN materia m ON m.clave = d.claveMateria																
														WHERE registro = '".$registro."' AND periodo = ".$periodo);
	    ...
	    /*
		 * Esta funcion sirve para obtener el nombre de la materia de los derechos de pasante de un alumno
		*/
		public function get_derechoPasante_PDF($idDP)
		{
		  $xextraordinarios	= new Xextraordinarios();

		  
		  $result = $xextraordinarios->find_all_by_sql("SELECT m.nombre
														FROM materia m
														JOIN derechopasante d ON d.claveMateria = m.clave
														WHERE d.idDerecho = ".$idDP);
