...
require_once "library/kumbia/generator/main.php";
require_once "library/kumbia/xml/main.php";
require_once "kumbia.php";

if(!dbBase::load_driver()){
	Flash::warning('No se pudo cargar el driver de la base de datos definido en config/config.ini');
	return false;
}
...
if($kumbia_config = Config::read('core.ini')){
	...
	foreach($extensions as $extension){
		...
		require_once "library/{$ex[0]}/{$ex[1]}/main.php";
	}
}
...
?>\n";		file_put_contents("controllers/$a"."_controller.php", $file);
			print "El controlador '$a' se creó correctamente\n";
		}
	}
	return true;
}
...
?>\n";		file_put_contents("models/$a.php", $file);
			print "El modelo '$a' se creó correctamente\n";
		}
	}
	return true;
}
