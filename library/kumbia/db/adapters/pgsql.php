...
class DbPgSQL extends DbBase implements DbBaseInterface {
	 ...
	 */
	public function connect($dbhost='', $dbuser='', $dbpass='', $dbname='', $dbport=5432, $dbdsn=''){
		...
		if($this->id_connection = pg_connect("host={$dbhost} user={$dbuser} password={$dbpass} dbname={$dbname} port={$dbport}")){
			return true;
		} else {
			throw new DbException($this->error("No se puede conectar a la base de datos"), $this->no_error(), false);
			return false;
		}
	...
	function query($sqlQuery){
		...
		if($resultQuery = @pg_query($this->id_connection, $sqlQuery)){
			...
			throw new DbException($this->error(" al ejecutar <i>'$sqlQuery'</i>"), $this->no_error());
			return false;
		}
	}
