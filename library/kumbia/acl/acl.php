 ...
 * @see AclException
 */
include "library/kumbia/acl/exception.php";

/**
 * @see AclRole
 */
include "library/kumbia/acl/role/role.php";

/**
 * @see AclResource
 */
include "library/kumbia/acl/resource/resource.php";
...
class Acl {
	 ...
	 * $access_inherits = Nombre del Role del cual hereda permisos � array del grupo
	...
	public function add_inherit($role, $role_to_inherit){
		...
		if($role_to_inherit!=''){
			if(is_array($role_to_inherit)){
				foreach($role_to_inherit as $rol_in){
					...
					if(!in_array($rol_in, $this->roles_names)){
						throw new AclException("El Rol '{$rol_in}' no existe en la lista");
						return false;
					}
				...
				if(!in_array($role_to_inherit, $this->roles_names)){
					throw new AclException("El Rol '{$role_to_inherit}' no existe en la lista");
					return false;
				}
			...
			throw new AclException("Debe especificar un rol a heredar en Acl::add_inherit");
			return false;
		}
	}
	...
	public function allow($role, $resource, $access){
		...
		if(!in_array($role, $this->roles_names)){
			throw new AclException("No existe el rol '$role' en la lista");
			return;
		}

		if(!in_array($resource, $this->resources_names)){
			throw new AclException("No existe el resource '$resource' en la lista");
			return;
		}
		...
		if(is_array($access)){
			foreach($access as $acc){
				if(!in_array($acc, $this->access_list[$resource])){
					throw new AclException("No existe el acceso '$acc' en el resource '$resource' de la lista");
					return false;
				}
			}
			foreach($access as $acc){
	...
	public function deny($role, $resource, $access){
		...
		if(is_array($access)){
			foreach($access as $acc){
				if(!in_array($acc, $this->access_list[$resource])){
					throw new AclException("No existe el acceso '$access' en el resource '$resource' de la lista");
					return false;
				}
			}
			foreach($access as $acc){
	...
	public function is_allowed($role, $resource, $access_list){
		...
		if(!in_array($role, $this->roles_names)){
			throw new AclException("El rol '$role' no existe en la lista en acl::is_allowed");
			return false;
		}
		if(!in_array($resource, $this->resources_names)){
			throw new AclException("El resource '$resource' no existe en la lista en acl::is_allowed");
			return false;
		}
		if(is_array($access_list)){
			foreach ($access_list as $access) {
				if(!in_array($access, $this->access_list[$resource])){
					throw new AclException("No existe en acceso '$access' en el resource '$resource' en acl::is_allowed");
					return false;
				}
			}
		} else {
			if(!in_array($access_list, $this->access_list[$resource])){
				throw new AclException("No existe en acceso '$access_list' en el resource '$resource' en acl::is_allowed");
				return false;
			}
		}
