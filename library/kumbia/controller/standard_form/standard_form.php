...
/**
 * @see StandardFormException
 */
require_once "library/kumbia/controller/standard_form/exception.php";
...
abstract class StandardForm extends Controller  {
	...
	public public function report(){
		...
		if(!kumbia::is_model($this->source)){
			throw new StandardFormException('No hay un modelo "'.$this->source.'" para hacer la operaci&oacute;n de reporte');
			$this->_create_model();
		...
		require_once "library/kumbia/report/report.php";
	...
	public function update(){
		...
		if(!kumbia::is_model($this->source)){
			throw new StandardFormException('No hay un modelo "'.$this->source.'" para hacer la operaci&oacute;n de actualizaci&oacute;n');
			$this->_create_model();
			return $this->route_to('action: index');
		}
	...
	public function delete(){
		...
		if(!kumbia::is_model($this->source)){
			throw new StandardFormException('No hay un modelo "'.$this->source.'" para hacer la operaci&oacute;n de eliminaci&oacute;n');
			$this->_create_model();
			return $this->route_to('action: index');
		}
	...
	*/
    ...
    protected function set_size_browse(){
        ...
        if(count($opt) > 0){
            ...
            throw new StandardFormException("No indico ning&uacute;n campo para aplicar este m&eacute;todo");
        }
    }
	/**
