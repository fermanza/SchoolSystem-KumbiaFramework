...
abstract class Generator extends Object {
	...
	static function dump_field_information($form){
		...
		if(!isset($_SESSION['dumps'][$_SESSION['KUMBIA_PATH']])){
			$_SESSION['dumps'][$_SESSION['KUMBIA_PATH']] = array();
		}
		if(!isset($_SESSION['dumps'][$_SESSION['KUMBIA_PATH']][$form['source'].$form['type']])){
			$_SESSION['dumps'][$_SESSION['KUMBIA_PATH']][$form['source'].$form['type']] = null;
		}
	...
	static function build_form($form, $scaffold=false){
		...
		require_once "library/kumbia/generator/components.php";
		...
		//Standard Forms
		if($form['type']=='standard'){
			include_once "library/kumbia/generator/standard.build.php";
