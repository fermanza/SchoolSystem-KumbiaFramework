...
class Report {
	...
	static function generate($form){
		...
		foreach($form['components'] as $name => $com){
			...
			if(!$com['notReport']){
				if(isset($com['caption'])&&$com['caption']){
					$headerArray[$n] = str_replace("&oacute;", "�", $com['caption']);
					$headerArray[$n] = str_replace("&aacute;", "�", $headerArray[$n]);
					$headerArray[$n] = str_replace("&eacute;", "�", $headerArray[$n]);
					$headerArray[$n] = str_replace("&iacute;", "�", $headerArray[$n]);
					$headerArray[$n] = str_replace("&uacute;", "�", $headerArray[$n]);
		...
		if($_REQUEST['reportType']!='html'){
			$title = str_replace("&oacute;", "�", $form['caption']);
			$title = str_replace("&aacute;", "�", $title);
			$title = str_replace("&eacute;", "�", $title);
			$title = str_replace("&iacute;", "�", $title);
			$title = str_replace("&uacute;", "�", $title);
		} else {
			$title = $form['caption'];
		}
		...
		switch($_REQUEST['reportType']){
			case 'pdf':
				require_once "library/kumbia/report/format/pdf.php";
				pdf($result, $sumArray, $title, $weightArray, $headerArray);
			break;
			case 'xls':
				require_once "library/kumbia/report/format/xls.php";
				xls($result, $sumArray, $title, $weightArray, $headerArray);
			break;
			case 'html':
				require_once "library/kumbia/report/format/htm.php";
				htm($result, $sumArray, $title, $weightArray, $headerArray);
			break;
			case 'doc':
				require_once "library/kumbia/report/format/doc.php";
